# # PIG/diffusion/infer.py
# # -*- coding: utf-8 -*-
# """
# Zero-shot personalization (Img2Img) + DeepPromptHyperNet personalization + LoRA personalization.

# 기본 모드:
# - SD v1.5 Img2Img
# - 사용자 임베딩 u -> proj_u2txt -> e_user
# - 토크나이저에 "<usr>" 토큰을 추가하고, 해당 임베딩을 e_user로 치환
# - 사용자의 최근 시청 영화 포스터(Poster)를 img2img로 개인화 생성

# DPT 모드(--use_dpt):
# - 학습된 DeepPromptHyperNet(예: dpt_hypernet_ep5.pt)을 로드
# - prompt_embeds = [dp_tokens(K), base_text_embeds(77), e_user(1)] 로 구성
# - negative_prompt_embeds 도 동일 길이(K+77+1)로 구성하여 CFG 사용 시 길이 불일치 방지
# - proj_u2txt: DPT 전용 체크포인트 사용 가능(--proj_ckpt_dpt)

# LoRA 모드(--use_lora):
# - 학습된 LoRA/UNet 어댑터를 로드
#   - 표준 파일명(pytorch_lora_weights.safetensors 등) 먼저 시도
#   - 없으면 diffusion_pytorch_model.safetensors를 PEFT 백엔드로 강제 로드
# - proj_u2txt: LoRA 전용 체크포인트 사용 가능(--proj_ckpt_lora)
# """
...
# RAW_KG = ROOT / "data" / "raw" / "ml-latest"    # Kaggle movies_metadata.csv (여기에 둔다고 가정)
...
# # ---------------- DPT Helpers ----------------
# @torch.no_grad()
# def build_dpt_pos_neg_embeds(pipe: StableDiffusionImg2ImgPipeline,
#                              prompt: str,
#                              neg_prompt: str,
...
#                              device: str):
#     """
#     양성/음성 임베딩을 동일한 길이(K+77+1)로 생성하여 반환.
#     Return:
#       pos_cond: [1, K+77+1, 768] = [dp_tokens, base_pos, e_user]
#       neg_cond: [1, K+77+1, 768] = [zeros(K), base_neg, zeros(1)]
#     """
...
#         raise FileNotFoundError(f"DeepPromptHyperNet checkpoint not found: {ckpt_path}")
#     state = torch.load(str(ckpt_path), map_location=device)
#     hyper.load_state_dict(state, strict=True)
#     hyper.eval()

#     dp_tokens = hyper(user_vec.unsqueeze(0)).to(text_encoder.dtype)  # [1,K,768]

#     # base text embeds (pos / neg)  [1,77,768]
...
#     # concat
#     pos_cond = torch.cat([dp_tokens, base_pos, e_user], dim=1)  # [1,K+77+1,768]
...
#     인자로 들어온 r/alpha/target_modules/lora_scale/lora_dropout은 config.json보다 우선 적용.
#     """
...
#             # diffusers>=0.20에서 제공 (버전에 따라 없을 수 있음)
#             try:
...
#     # 2-2) 현재 UNet에 동일 타깃 모듈로 LoRA 장착
#     lcfg = LoraConfig(
#         task_type="FEATURE_EXTRACTION",   # 문자열로 버전 독립 처리
#         r=r,
...
#     peft_unet = get_peft_model(unet, lcfg)  # 임시 래퍼
#     peft_unet.to(device)

#     # 2-3) safetensors 로드 & 키 매핑
...
#     # ---- 여기부터 dtype 정렬 워크어라운드 ----
#     # 시간 임베딩이 float32로 만들어지는 경로와의 dtype mismatch 방지를 위해
#     # UNet은 fp32로 고정 (가장 안정적)
#     merged_unet.to(device=device, dtype=torch.float32)
#     pipe.unet = merged_unet

#     # ✅ 핵심: 파이프라인 전체 dtype을 float32로 통일 (autocast도 float32로 동작)
...
#       2) --use_dpt → --proj_ckpt_dpt, 없으면 체크포인트 폴더의 proj_u2txt_dpt_ep5.pt 추정
#       3) --use_lora → --proj_ckpt_lora, 없으면 체크포인트 폴더의 proj_u2txt_lora_ep5.pt 추정
...
#         # 추정 경로
#         p = (CHECKPOINT_DIR / "proj_u2txt_dpt_ep5.pt").resolve()
#         if p.exists():
#             logger.info(f"[proj] Using inferred DPT proj: {p}")
#             return p
#         logger.warning(f"[proj] Inferred DPT proj not found: {p} → fallback")
...
#         # 추정 경로
#         p = (CHECKPOINT_DIR / "proj_u2txt_lora_ep5.pt").resolve()
#         if p.exists():
#             logger.info(f"[proj] Using inferred LoRA proj: {p}")
#             return p
#         logger.warning(f"[proj] Inferred LoRA proj not found: {p} → fallback")
...
#             f"Tried: --proj_ckpt / mode-specific / PROJ_U2TXT_CKPT ({PROJ_U2TXT_CKPT})"
#         )
...
#     # DPT options
#     ap.add_argument("--use_dpt", action="store_true", help="Use DeepPromptHyperNet for personalization.")
#     ap.add_argument("--dpt_ckpt", type=str, default="", help="Path to dpt_hypernet checkpoint (e.g., .../dpt_hypernet_ep5.pt)")
#     ap.add_argument("--K", type=int, default=8, help="Number of deep prompt tokens; MUST match training.")
#     # LoRA options
...
#     # 1) Load pipeline (버전 호환을 위해 dtype 인자는 from_pretrained에 넘기지 않고 이후 .to로 이동)
#     pipe = StableDiffusionImg2ImgPipeline.from_pretrained(SD15_MODEL_ID)
#     pipe.scheduler = DPMSolverMultistepScheduler.from_config(pipe.scheduler.config)
#     pipe = pipe.to(device)
#     # safety checker는 기본 유지 (공개 환경이면 켠 채로 사용 권장)
...
#     # 6) Precompute DPT embeds if needed
#     pos_embeds = None
#     neg_embeds = None
#     if args.use_dpt:
#         if not args.dpt_ckpt:
#             raise ValueError("--use_dpt requires --dpt_ckpt path.")
...
#                     generator=gen,
#                 )
#                 suffix = f"dpt_seed{seed_i}_str{args.strength:.2f}_{i:02d}.png"
#             else:
...
# === NEW: DPT utils & optional masking processors (match training) ===
